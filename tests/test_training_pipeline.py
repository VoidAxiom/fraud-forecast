...
def test_data_loader_no_future_leakage(
    ...
    """Verify hand-computed ground-truth window counts use strict prior orders."""
    assert tmp_path.exists()
    placed_at = [
    ...
    def fake_read_sql_query(
        ...
        def count_prior_window(row: pd.Series, window_seconds: int) -> int:
            # Phase 5 contract: velocity features reflect orders strictly BEFORE placed_at; tied-timestamp peer behavior is tracked at VOI-251 (production SQL bug).
            t = row["placed_at"]
